...
class BookFormTestCase(TestCase):
    ...
    def test_blank_author_is_short(self):
    ...
    def test_overlong_is_invalid(self):
    ...
    def test_13_isbn_is_valid(self):
        self.form_input['isbn'] = "1234567890123"
        form = BookForm(data = self.form_input)
        self.assertTrue(form.is_valid())

    def test_12_isbn_is_invalid(self):
        self.form_input['isbn'] = "123456789012"
        form = BookForm(data = self.form_input)
        self.assertFalse(form.is_valid())

    def test_11_isbn_is_invalid(self):
        self.form_input['isbn'] = "12345678901"
        form = BookForm(data = self.form_input)
        self.assertFalse(form.is_valid())

    def test_10_isbn_is_valid(self):
        self.form_input['isbn'] = "1234567890"
        form = BookForm(data = self.form_input)
        self.assertTrue(form.is_valid())

    def test_short_isbn_is_invalid(self):
        self.form_input['isbn'] = "123456789"
        form = BookForm(data = self.form_input)
        self.assertFalse(form.is_valid())

    def test_13_with_string_isbn_is_invalid(self):
        self.form_input['isbn'] = "1234567b90123"
        form = BookForm(data = self.form_input)
        self.assertFalse(form.is_valid())

    def test_10_with_string_isbn_is_invalid(self):
        self.form_input['isbn'] = "1234567b90"
        form = BookForm(data = self.form_input)
        self.assertFalse(form.is_valid())
